...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        // Handle logo upload
        $logo_path = $platform['logo_path']; // Keep existing logo by default
        $old_logo_to_delete = null;
        
        if (isset($_FILES['logo']) && $_FILES['logo']['error'] === UPLOAD_ERR_OK) {
            $upload_result = upload_file($_FILES['logo']);
            if ($upload_result['success']) {
                $old_logo_to_delete = $platform['logo_path'];
        ...
        if (empty($errors)) {
            try {
                ...
                if ($old_logo_to_delete) {
                    delete_file(basename($old_logo_to_delete));
                }
                
                set_flash('success', 'Platform updated successfully!');
                redirect('index.php');
            } catch (PDOException $e) {
                error_log("Platform update error: " . $e->getMessage());
                $errors[] = 'Error updating platform. Please try again.';
                
                // Clean up newly uploaded file if database update fails
                if ($logo_path !== $platform['logo_path']) {
        ...
        <main class="flex-1 p-8">
            ...
            <div class="mb-8 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600 mt-2">Update platform information</p>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <form method="POST" enctype="multipart/form-data" class="p-8 space-y-6">
                    ...
                    <!-- Current Logo Display and Upload -->
                    <div>
                        ...
                        <?php if ($platform['logo_path']): ?>
                            <div class="mb-4 flex items-start space-x-4">
                                <img src="/<?= e($platform['logo_path']) ?>" alt="Current logo" class="w-32 h-32 rounded-lg object-cover border-2 border-gray-200 shadow">
                                <div class="flex-1">
                                    ...
                                    <label class="flex items-center space-x-2 cursor-pointer">
                                        ...
                                        >
                                        <span class="text-sm text-red-600 font-medium">Remove current logo</span>
                                    </label>
                                </div>
                            </div>
                        <?php endif; ?>
                        ...
                        <p class="mt-2 text-sm text-gray-500">
                            <?= $platform['logo_path'] ? 'Upload a new logo to replace the current one.' : 'Upload a logo image.' ?>
            ...
            <div class="mt-6 bg-white rounded-lg shadow p-6">
                <h3 class="text-sm font-semibold text-gray-700 mb-3">Platform Metadata</h3>
                <div class="grid md:grid-cols-3 gap-4 text-sm">
                    <div>
                        <span class="text-gray-500">Created:</span>
                        <span class="font-medium text-gray-900 ml-2"><?= format_datetime($platform['created_at']) ?></span>
                    </div>
                    <div>
                        <span class="text-gray-500">Last Updated:</span>
                        <span class="font-medium text-gray-900 ml-2"><?= format_datetime($platform['updated_at']) ?></span>
                    </div>
                    <div>
                        <span class="text-gray-500">Platform ID:</span>
                        <span class="font-medium text-gray-900 ml-2">#<?= $platform['id'] ?></span>
                    </div>
                </div>
            </div>
        </main>
