...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("SELECT id FROM tutorials WHERE slug = ?");
            $stmt->execute([$slug]);
            if ($stmt->fetch()) {
    ...
    // Insert tutorial if no errors
    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("
                INSERT INTO tutorials (title, slug, content, category, image_path, display_order, is_published, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            ");
            ...
            set_flash('success', 'Tutorial created successfully!');
            redirect('index.php');
        } catch (PDOException $e) {
            error_log("Tutorial creation error: " . $e->getMessage());
            $errors[] = 'Error creating tutorial. Please try again.';
        ...
        <main class="flex-1 p-8">
            ...
            <div class="mb-8">
                <div class="flex items-center space-x-3 mb-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-gray-800">Create Tutorial</h1>
                        <p class="text-gray-600 mt-2">Add a new educational guide</p>
                    </div>
                </div>
            </div>
